...
class ScatterPlotFactory(PlotType):
    ...
    def create_trace(settings):
        ...
        fig = px.scatter(
            x=settings.x,
            y=settings.y,
            color=settings.data_defined_colors if settings.data_defined_colors else None
        )

        for current, _ in enumerate(fig.data):
            fig.data[current].ids = settings.feature_ids
