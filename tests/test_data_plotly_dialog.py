...
def init_layout():
    QgsApplication.layoutItemRegistry().addLayoutItemType(plot_item_metadata)


def test_get_settings(qgis_iface: QgisInterface):
    """
    Test retrieving settings from the dialog
    """
    dialog = DataPlotlyPanelWidget(None, override_iface=qgis_iface)
    settings = dialog.get_settings()
    # default should be scatter plot
    assert settings.plot_type == "scatter"

    dialog.set_plot_type("violin")
    settings = dialog.get_settings()
    # default should be scatter plot
    assert settings.plot_type == "violin"


def test_set_default_settings(qgis_iface: QgisInterface):
    """
    ...
    assert dialog.get_settings().plot_type == settings.plot_type
    for k in settings.properties:
        if k in [
            "x",
            "y",
            "z",
    ...
    for k in settings.layout:
        assert dialog.get_settings().layout[k] == settings.layout[k]


def test_settings_round_trip(data: Path, qgis_iface: QgisInterface):
    """
    Test setting and retrieving settings results in identical results
    """
    layer_path = str(data.joinpath("test_layer.geojson"))
    ...
    settings.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_MARKER_SIZE, QgsProperty.fromExpression("5+64")
    )
    settings.data_defined_properties.setProperty(
    ...
    settings.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_TITLE, QgsProperty.fromExpression("concat('my', '_title_', @some_var)")
    )
    settings.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_LEGEND_TITLE, QgsProperty.fromExpression("concat('my', '_legend_', @some_var)")
    )
    settings.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_X_TITLE, QgsProperty.fromExpression("concat('my', '_x_axis_', @some_var)")
    )
    settings.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_Y_TITLE, QgsProperty.fromExpression("concat('my', '_y_axis_', @some_var)")
    )
    settings.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_Z_TITLE, QgsProperty.fromExpression("concat('my', '_z_axis_', @some_var)")
    )
    settings.data_defined_properties.setProperty(
    ...
    dialog2 = DataPlotlyPanelWidget(None, override_iface=qgis_iface)
    dialog2.set_settings(settings)

    # print('set settings')

    assert dialog2.get_settings().plot_type == settings.plot_type
    for k in settings.properties:
        # print(k)
        if k in ["x", "y", "z", "additional_hover_text", "featureIds", "featureBox", "custom"]:
            continue
        assert dialog2.get_settings().properties[k] == settings.properties[k]
    for k in settings.layout:
        assert dialog2.get_settings().layout[k] == settings.layout[k]
    assert dialog2.get_settings().source_layer_id == vl3.id()
    assert dialog2.get_settings().data_defined_properties.property(
    ...
    dialog3 = DataPlotlyPanelWidget(None, override_iface=qgis_iface)
    # print('dialog 2')
    dialog3.set_settings(settings)
    # print('set settings')

    assert dialog3.get_settings().plot_type == settings.plot_type
    for k in settings.properties:
        # print(k)
        assert dialog3.get_settings().properties[k] == settings.properties[k]
    assert dialog3.get_settings().properties == settings.properties
    for k in settings.layout:
        # print(k)
        assert dialog3.get_settings().layout[k] == settings.layout[k]
...
def test_settings_round_trip_secondary(data: Path, qgis_iface: QgisInterface):
    """
...
@pytest.mark.skip("causing crash?")
def test_read_write_project(data: Path, qgis_iface: QgisInterface, output_dir: Path):
    """
    ...
    # def read(doc):
    #    nonlocal read_triggered
    #    assert res.read_from_project(doc))
    #    assert res.plot_type, 'violin')
    ...
    assert dialog.x_combo.expression() == '"Ca"'


def test_read_write_project_with_layout():
    """
    ...
    assert len(layout_plot.plot_settings) == 1
    ...
    assert len(layout_plot.plot_settings) == 2
    ...
    assert plot_property_panel.ptype == "violin"
    ...
    assert plot_property_panel.ptype == "bar"
    plot_property_panel.acceptPanel()
    plot_property_panel.destroy()
    ...
    assert len(layout_plot.plot_settings) == 1
    assert layout_plot.plot_settings[0].plot_type == "bar"

    layout_plot.remove_plot(0)
    assert len(layout_plot.plot_settings) == 0

    # read xml
    layout2 = QgsPrintLayout(project)
    assert layout2.readXml(element, xml_doc, QgsReadWriteContext())
    layout_plot2 = layout2.itemById(plot_item_id)
    assert layout_plot2

    assert len(layout_plot2.plot_settings) == 2
    assert layout_plot2.plot_settings[0].plot_type == "violin"
    assert layout_plot2.plot_settings[1].plot_type == "bar"


def test_move_chart_in_layout():
    """
    ...
    assert manager.addLayout(layout)
    ...
    assert layout_plot.plot_settings[0].plot_type == "violin"
    assert layout_plot.plot_settings[1].plot_type == "bar"
    # move up second item
    plot_dialog.plot_list.setCurrentRow(1)
    plot_dialog.move_up_plot()
    assert layout_plot.plot_settings[0].plot_type == "bar"
    assert layout_plot.plot_settings[1].plot_type == "violin"
    ...
    assert manager.removeLayout(layout)


def test_duplicate_chart_in_layout():
    """
    ...
    plot_property_panel.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_FILTER, QgsProperty.fromExpression('"mid">20')
    )
    plot_property_panel.acceptPanel()
    plot_property_panel.destroy()
    ...
    assert layout_plot.plot_settings[0].plot_type == "violin"
    assert layout_plot.plot_settings[1].plot_type == "violin"
    assert (layout_plot.plot_settings[0]).properties["x_name"] == "mid"
    assert (layout_plot.plot_settings[1]).properties["x_name"] == "mid"
    assert layout_plot.plot_settings[0].data_defined_properties.property(
        PlotSettings.PROPERTY_FILTER
    ) == QgsProperty.fromExpression('"mid">20')
    assert layout_plot.plot_settings[1].data_defined_properties.property(
        PlotSettings.PROPERTY_FILTER
    ) == QgsProperty.fromExpression('"mid">20')
    ...
    plot_property_panel.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_FILTER, QgsProperty.fromExpression('"qid">20')
    )
    plot_property_panel.acceptPanel()
    plot_property_panel.destroy()
    ...
    assert layout_plot.plot_settings[1].plot_type == "bar"
    assert (layout_plot.plot_settings[0]).properties["x_name"] == "mid"
    assert (layout_plot.plot_settings[1]).properties["x_name"] == "qid"
    assert layout_plot.plot_settings[0].data_defined_properties.property(
        PlotSettings.PROPERTY_FILTER
    ) == QgsProperty.fromExpression('"mid">20')
    assert layout_plot.plot_settings[1].data_defined_properties.property(
    ...
    assert plot_property_panel.ptype == "scatter"
    plot_property_panel.x_combo.setExpression("uid")
    plot_property_panel.data_defined_properties.setProperty(
        PlotSettings.PROPERTY_FILTER, QgsProperty.fromExpression('"uid">20')
    )
    plot_property_panel.acceptPanel()
    plot_property_panel.destroy()

    assert layout_plot.plot_settings[0].plot_type == "scatter"
    assert layout_plot.plot_settings[1].plot_type == "bar"
    assert (layout_plot.plot_settings[0]).properties["x_name"] == "uid"
    assert (layout_plot.plot_settings[1]).properties["x_name"] == "qid"
    assert layout_plot.plot_settings[0].data_defined_properties.property(
        PlotSettings.PROPERTY_FILTER
